...
2. **동일 이미지 중복 사용 금지**: 하나의 포스트에서 사용한 배경 이미지를 다른 포스트의 썸네일에 재사용하지 않는다. 각 포스트는 주제에 맞는 **고유한** 배경 이미지를 사용해야 한다. 카테고리가 같더라도 구분되는 이미지를 선택하거나, 같은 카테고리 내에서 교대로 다른 사진을 사용한다.
