...
2. **동일 이미지 중복 사용 금지**: 기존 포스트 전체를 분석하여 이미 사용된 photo_category는 가급적 재사용하지 않는다.
   - 신규 포스팅 시 **전체 포스트 대상** photo_category 사용 횟수를 집계
   - 사용 횟수가 적은 photo_category를 우선 배정
   - 미사용 photo_category가 있으면 반드시 그것을 먼저 사용
