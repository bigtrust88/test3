...
2. **동일 이미지 중복 사용 금지**: 기존 포스트 전체에서 사용된 적 없는 Unsplash 사진을 우선 사용한다.
   - 배경 사진 선택은 **포스트 주제와 무관** — 중복 방지가 우선
   - 코드가 자동으로 미사용 사진을 순서대로 배정 (Claude가 결정하지 않음)
   - 같은 실행(run) 내 썸네일 배경과 본문 이미지도 서로 다른 사진 사용
   - 썸네일의 주제 표현은 텍스트(헤드라인/서브텍스트)로 한다
