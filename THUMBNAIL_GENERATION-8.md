...
2. **동일 이미지 중복 사용 금지**: 기존 포스트 전체를 분석하여 아직 사용되지 않은 photo_category를 우선 사용한다.
   - 신규 포스팅 시 전체 포스트의 photo_category 사용 현황을 집계
   - **미사용 photo_category가 있으면 반드시 그것부터 사용**
   - 미사용이 없으면 가장 적게 사용된 photo_category 선택
   - 같은 실행(run) 내 3개 포스트는 반드시 서로 다른 photo_category 사용
   - 단, 배경 사진은 반드시 **포스트 주제/컨셉에 맞는 것**으로 선택 (사용 가능한 목록 내에서 주제에 가장 어울리는 것 고름)
